...
if __name__ == '__main__':
    ...
    if (len(parm) >= 2):
        ...
        p_runMode = parm[1]
    if (len(parm) >= 3):
        qLog.log('info', main_id, 'parm[2] = ' + str(parm[2] ))
        p_screen  = parm[2]
    ...
    while (break_flag == False):
        ...
        if ((runMode == 'auto')      and ((time.time() - main_start) > 180*60) \
        and (str(parm[1])[:3] == 'bgm')                                      ) \
        or ((runMode == 'reboot15')  and ((time.time() - main_start) >  15*60)) \
        ...
        if  ((p_runMode == 'bgvideo') and (p_screen == 'auto')) \
        or  ((p_runMode[:3] == 'bgv') and (p_screen == 'all')):
            ...
            for s in range(qGUI.screen_count):
                ...
                if (process_pool[p] is None):
                    time.sleep(5.00)
                    parm[2] = str(s) # screen s
                    qLog.log('info', main_id, '(re)start ' + str(parm[0]) + ' ' + str(parm[1]) + ' ' + str(parm[2]))
                    process_parm[p] = parm
                    if (os.name == 'nt'):
